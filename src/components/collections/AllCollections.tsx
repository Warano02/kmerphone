...
function AllCollections({ size }: { size?: number }) {
    return (
        <div>
            ...
            <div className="w-full flex flex-col  sm:px-4 sm:grid grid-cols-3 gap-2">
                {collections.slice(0, size || collections.length).map((collection, i) => (
                    <Link to={`/collections/${i + 1 !== collections.length ? collection.name.toLocaleLowerCase() : ""}`} key={i} className="w-full sm:w-96 h-[412px] relative">
